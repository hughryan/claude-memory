...
class TestInferTags:
    ...
    def test_infer_bugfix_from_fix(self):
        ...
        from claude_memory.memory import _infer_tags
...
class TestHybridWeightWiring:
    ...
    def test_memory_uses_config_weight(self, monkeypatch):
        ...
        import importlib
        monkeypatch.setenv("CLAUDE_MEMORY_HYBRID_VECTOR_WEIGHT", "0.7")

        # Force reload of settings and vectors module
        from claude_memory import config, vectors
        config.settings = config.Settings()
        importlib.reload(vectors)

        from claude_memory.vectors import HybridSearch
