"""
ClaudeMemory Server - AI Memory System with Semantic Understanding
...
try:
    ...
    from claude_memory.models import Memory, Rule, CodeEntity
...
if os.getenv('CLAUDE_MEMORY_STRUCTURED_LOGS'):
    handler = logging.StreamHandler()
    handler.setFormatter(StructuredFormatter())
    cm_logger = logging.getLogger('claude_memory')
...
# Default project path (ONLY used if CLAUDE_MEMORY_PROJECT_ROOT is explicitly set)
...
def _missing_project_path_error() -> Dict[str, Any]:
    ...
    return {
        ...
        "message": (
            ...
            "The memory system serves multiple realms - you must specify which project's memories to access. "
...
def _get_storage_for_project(project_path: str) -> str:
    ...
    return str(Path(project_path) / ".claude-memory" / "storage")
...
async def _track_task_context(ctx: ProjectContext) -> None:
    ...
    async with _task_contexts_lock:
        ...
        if not getattr(task, "_cm_ctx_tracked", False):
            setattr(task, "_cm_ctx_tracked", True)
...
async def get_project_context(project_path: Optional[str] = None) -> ProjectContext:
    ...
    if not project_path:
        raise ValueError("project_path is required when CLAUDE_MEMORY_PROJECT_ROOT is not set")
...
logger.info(f"ClaudeMemory Server initialized (default storage: {storage_path})")
...
async def _get_linked_projects_summary(ctx: ProjectContext) -> List[Dict[str, Any]]:
    ...
    for link in links:
        ...
        linked_storage = Path(linked_path) / ".claude-memory" / "storage"
...
def _build_briefing_message(
    ...
    """
    message_parts = [f"ClaudeMemory ready. {stats['total_memories']} memories stored."]
...
SKIP_DIRS = {
    ...
    '*.egg-info', '.mypy_cache', '.coverage', 'htmlcov', '.claude-memory'
}
...
async def index_project(
    ...
    try:
        ...
        from claude_memory.code_indexer import CodeIndexManager, is_available
...
async def consolidate_linked_databases(
    ...
    Args:
        archive_sources: Rename source .claude-memory to .claude-memory.archived
        project_path: Project root
    """
    if not project_path and not _default_project_path:
        return _missing_project_path_error()
...
async def _context_resource_impl(project_path: str, db_manager: DatabaseManager) -> str:
    ...
    try:
        ...
        return f"""# Memory Project Context
...
@mcp.resource("memory://warnings/{project_path}")
...
@mcp.resource("memory://failed/{project_path}")
...
@mcp.resource("memory://rules/{project_path}")
...
@mcp.resource("memory://context/{project_path}")
...
async def triggered_context_resource(file_path: str) -> str:
    ...
    Usage: Access memory://triggered/src/auth/service.py
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ClaudeMemory Server")
    parser.add_argument(
        "--transport", "-t",
        choices=["stdio", "sse"],
        default="stdio",
    ...
    logger.info("Starting ClaudeMemory server...")
