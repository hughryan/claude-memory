...
When Claude Memory memory tools are available, you MUST follow this protocol. Memory without discipline is noise.

**Core principle:** Check before you change, record what you decide, track whether it worked.

## Tool Detection

First, verify Claude Memory tools are available:

```
Look for these tools in your available tools:
- mcp__claudememory__get_briefing
- mcp__claudememory__context_check
- mcp__claudememory__remember
- mcp__claudememory__record_outcome
- mcp__claudememory__link_memories
- mcp__claudememory__trace_chain
- mcp__claudememory__get_graph
- mcp__claudememory__find_code
- mcp__claudememory__analyze_impact
- mcp__claudememory__index_project
```
...
```
IMMEDIATELY when you have claudememory tools:
...
mcp__claudememory__context_check(description="what you're about to do")

OR for specific files:

mcp__claudememory__recall_for_file(file_path="path/to/file")
```
...
memory_result = mcp__claudememory__remember(
...
The Protocol is now ENFORCED, not advisory:
...
This is cryptographic proof you consulted the memory system.
...
```
mcp__claudememory__unlink_memories(
    source_id=<id>,
    target_id=<id>,
    relationship="led_to"
)
...
If the project uses OpenSpec (spec-driven development), the `openspec-memory-bridge` skill provides bidirectional integration:
...
See the `openspec-memory-bridge` skill for full workflow details.
...
```
mcp__claudememory__recall(topic="authentication", condensed=True)
```
...
**Index your codebase:**
```
mcp__claudememory__index_project()  # Index all code entities
```

**Search code semantically:**
```
mcp__claudememory__find_code(query="user authentication")
```

**Analyze change impact:**
```
mcp__claudememory__analyze_impact(entity_name="UserService.authenticate")
```
...
This is non-negotiable when Claude Memory tools are available.
