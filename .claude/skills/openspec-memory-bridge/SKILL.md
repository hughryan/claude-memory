---
name: openspec-memory-bridge
description: Bridges OpenSpec (spec-driven development) with Claude Memory-MCP memory - auto-imports specs, informs proposals with past outcomes, converts archived changes to learnings
---

# OpenSpec-Claude Memory Bridge

## Overview

This skill creates a bidirectional bridge between:
- **OpenSpec**: Spec-driven development with formal change proposals
- **Claude Memory-MCP**: AI memory system with semantic search and outcome tracking

**The feedback loop:**
```
OpenSpec specs ──────► Claude Memory patterns/rules
       ▲                        │
       │                        ▼
...
1. Announce: "OpenSpec detected. Syncing specs to Claude Memory memory..."
...
4. **Create memories via remember_batch**
   ```
   mcp__claudememory__remember_batch(memories=[
       {
           "category": "pattern",
...
| OpenSpec Element | Claude Memory Category | Tags |
|-----------------|-----------------|------|
...
2. **Check applicable rules**
   ```
   mcp__claudememory__check_rules(
...
4. **If specific files are affected, check them**
   ```
   mcp__claudememory__recall_for_file(
...
2. **Find the original decision memory**
   ```
   mcp__claudememory__search_memories(
...
Claude: [Calls get_briefing()]
        ...
        OpenSpec detected. Syncing specs to Claude Memory memory...
...
Claude: [Workflow 3: Archive]
        ...
        The Claude Memory will remember this for future auth work.
```
...
```
mcp__claudememory__search_memories(query="[feature keywords]", tags=["openspec"])
```

### Rules not matching
Check rule triggers match your action descriptions:
```
mcp__claudememory__list_rules()
```
